...
public class Thread_Cert_Gest_User implements Runnable {
    ...
    // Constructor
    public Thread_Cert_Gest_User(String port_number) {
            this.port_number = Integer.valueOf(port_number);
    }

    @Override
    public void run() {
        System.out.println("\t> Thread_Cert_Gest_User RUN() called with data parameter = "+port_number);
        ...
        while(!isRequestEnded) { // tant que le traitement n'est pas fait
            ...
            while (!isPublic_key_client_received) { // tant que la clé du client n'a pas été reçu, on se met en attente de requête
                ...
                System.out.println("\n> Reception de la cle publique du gestionnaire user");
                ...
                System.out.println("En attente de la cle publique du gestionnaire ....\n");
                ...
                try {
                    ...
                    GestionClesRSA.sauvegardeClePublique(publicKey_client, AuthCertif_Srv.getPath()+"public_key_gest_user.bin");
            ...
            MyCertificat certificat = new MyCertificat("Auth_srv_en_bikini", "email@admin.com", "GestionUser_Srv", "v1", "58:57:59:58:57:59:58:57:59:58:57:59", "RSA", "localhost", dateStart.toString(), dateEnd.toString(), "O", publicKey_client,  MyAPI.getMac()); // O business
            // sauvegarde du certificat du client
            certificat.writeJSONCert(AuthCertif_Srv.getPath(), "cert_gest_user.json");
            ...
            while(!isCertificat_sended) {
                ...
                try {
                    ...
                    AuthCertif_Srv.getMap_entite().put("gest_user", true);
