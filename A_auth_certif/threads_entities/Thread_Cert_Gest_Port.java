...
public class Thread_Cert_Gest_Port implements Runnable {
    ...
    // Constructor
    public Thread_Cert_Gest_Port(String port_number, int id_port) {
            this.port_number = Integer.valueOf(port_number);
            this.nom_port = "port"+String.valueOf(id_port);
    }

    @Override
    public void run() {
        System.out.println("\t> Thread_Cert_Gest_Port RUN() called with data parameter = "+port_number+"\t"+nom_port);
        ...
        while(!isRequestEnded) { // tant que le traitement n'est pas fait
            ...
            while (!isPublic_key_client_received) { // tant que la clé du client n'a pas été reçu, on se met en attente de requête
                ...
                System.out.println("\n> Reception de la cle publique du gestionnaire de port");
                ...
                System.out.println("En attente de la cle publique du gestionnaire de port ....\n");
                ...
                try {
                    ...
                    GestionClesRSA.sauvegardeClePublique(publicKey_client, AuthCertif_Srv.getPath()+"public_key_gest_"+nom_port+".bin");
            ...
            MyCertificat certificat = new MyCertificat("Auth_srv_en_bikini", "email@admin.com", "gest_"+nom_port, "v1", "58:57:59:58:57:59:58:57:59:58:57:59", "RSA", "localhost", dateStart.toString(), dateEnd.toString(), "O", publicKey_client,  MyAPI.getMac()); // O business
            // sauvegarde du certificat du client
            certificat.writeJSONCert(AuthCertif_Srv.getPath(), "cert_gest_"+nom_port+".json");
            ...
            while(!isSymetric_key_sended) {
                ...
                try {
                    ...
                    System.out.println("encrypted_symetric_key sended "+ new String(encrypted_symetric_key)+" (size= "+donnees.length+")"); // 10
            ...
            while(!isCertificat_sended) {
                ...
                try {
                    ...
                    AuthCertif_Srv.getMap_entite_port().put(nom_port, true);
                    System.out.println("\nNew Map Entitie= \n"+AuthCertif_Srv.getMap_entite_port().toString()+"\n");
